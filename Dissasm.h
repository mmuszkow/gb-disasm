...
public:
	int			bank;
	uint16_t	pc;
	state(int bank, uint16_t pc) {
		this->bank = bank;
		this->pc = pc;
	}
} state;

void state_init(state* st, int bank, uint16_t pc) {
	st->bank = bank;
	st->pc = pc;
}
...
class Dissasm {
	...
	void jmp(uint16_t addr) {
		if(addr < 0x8000 && phy(addr) < r->total) {
			ret_stack.push(state(bank, pc + 3));
			pc = addr;
		} else {
			printf("Warning: Address too high, ignoring 0x%.8X\n", phy(addr));
			pc += 3;
		}
	}
	...
	void jmp(uint8_t addr) {
		if(addr < 0x8000) {
			ret_stack.push(state(bank, pc + 2));
			pc += ((char)addr) + 2;
		} else {
			printf("Warning: Address too high, ignoring 0x%.8X\n", phy(addr));
			pc += 2;
...
case 0xff:
	...
	void print_dump(const char* file_name) const {
	...
	void print_asm(const char* file_name) const {
		FILE* f = fopen(file_name, "w");
		if(f) {
			print_asm(f);
			fclose(f);
